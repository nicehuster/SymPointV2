...
class SetCriterion(nn.Module):
    ...
    def __init__(self, matcher,weight_dict, cfg,dn_no_lb=False):
        ...
        self.head = ContrastHead(cfg.contrast)


    def loss_labels(self, outputs, targets, indices, num_masks,prefix=''):
    ...
    def loss_masks(self, outputs, targets, indices, num_masks,prefix=''):
        ...
        losses = {
            "loss_mask"+prefix: sigmoid_ce_loss_jit(src_masks, target_masks, num_masks)*self.weight_dict["loss_mask"],
            "loss_dice"+prefix: dice_loss_jit(src_masks, target_masks, num_masks)*self.weight_dict["loss_dice"],
        }
        del src_masks
        del target_masks
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks,prefix=''):
        loss_map = {
        ...
        return loss_map[loss](outputs, targets, indices, num_masks, prefix)
    ...
    def forward(self, outputs, targets):
        ...
        if self.training:
            ...
            if "aux_outputs" in outputs['dn_out']:
                for i, aux_outputs in enumerate(outputs['dn_out']["aux_outputs"]):
                    for loss in self.losses:
                        l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_masks, prefix='_dn')
                        l_dict = {k + f"_{i}": v for k, v in l_dict.items()}
                        losses.update(l_dict)
